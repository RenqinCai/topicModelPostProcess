...
def simStatistics(corpusObj):
	...
	for articleWordStr in corpusObj.m_articleWordList:
		totalSim = 0
		for wordStr in corpusObj.m_wordMap.keys():
			wordObj = corpusObj.m_wordMap[wordStr]

			wordObj.m_articleTotalSim = (wordObj.m_wordSimMap[articleWordStr]-corpusObj.m_minSim)/(corpusObj.m_maxSim-corpusObj.m_minSim)
...
obtainTokenList(fileName, corpusObj)
readFile(fileName, corpusObj)

articleSimRatioList = simStatistics(corpusObj)
plotStatistics(articleSimRatioList)