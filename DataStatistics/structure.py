...
class _Stn:
	def __init__(self, name):
		...
		self.m_childDocMap = {} ##modelIndex:childName:simVal
		self.m_APMap = {} ###modelIndex:AP
		
	def getName(self):
		return self.m_name

	def addChild2Stn(self, childName, simVal, modelIndex):
		if modelIndex not in self.m_childDocMap.keys():
			self.m_childDocMap.setdefault(modelIndex, {})
			self.m_childDocMap[modelIndex].setdefault(childName, simVal)
		else:
			self.m_childDocMap[modelIndex].setdefault(childName, simVal)
